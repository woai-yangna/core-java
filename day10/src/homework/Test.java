...
public class Test{
    public static void main(String[] args) {
        ...
        Tester tester=new Tester("测试员","白居易","写代码bug没测试出来",12000);
...
class Account extends Staff{
    ...
    public void pay(Staff staff){
        if(staff instanceof Tester)
            System.out.println(this.getJob()+this.getName()+"给"+staff.getJob()+staff.getName()+"因为给客户造成损失，多扣除1000元，最后结算"+(staff.getMoney()-1000));
        else
            System.out.println(this.getJob()+this.getName()+"给"+staff.getJob()+staff.getName()+"结算工资"+staff.getMoney());
    }
}
class Hr extends Staff{
    ...
    public void fire(Staff staff){
        ...
        System.out.println(this.getJob()+this.getName()+"开除"+staff.getJob()+staff.getName()+"原因是"+staff.getCause());
    }
}
